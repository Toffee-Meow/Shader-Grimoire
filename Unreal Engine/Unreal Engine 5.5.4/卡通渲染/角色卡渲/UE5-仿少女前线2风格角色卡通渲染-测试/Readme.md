...
&emsp;&emsp;仿 **少女前线2**风格 角色卡通渲染 Shader/材质效果测试

&emsp;&emsp;默认渲染管线，基于 **材质蓝图** 与 **HLSL** 着色代码，实现 **NPR** 卡通光照 + **PBR** 物理材质融合渲染，复刻少女前线2风格角色卡通渲染效果
...
- 材质蓝图：
  - 颜色：基础色、贴图校色、基础调色
  - 卡通光照：Lambert模型、卡通二分（Ramp采样）
  - 阴影：SDF阴影
  - 质感：MatCap、Normal贴图、RMO贴图、PBR属性（RMS）
  - 高光：鼻尖高光
  - 描边：法线外扩描边
...
- Actor蓝图：
  - 光照方向控制SDF、添加描边
    - BP_Vepley.uasset：角色蓝图

- HLSL：
  - 去除引擎添加的色调映射、SDF阴影抗锯齿
    - Dummy.hlsl：规避色调映射模块
